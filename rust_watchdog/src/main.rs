...
pub struct AppState {
    ...
    pub(crate) creation_step: CreationStep,
    ...
    match parts[0] {
        ...
        "/plan" => {
            ...
            if requirement.is_empty() {
                ...
                // Collect data under lock, then DROP the lock before spawning to avoid deadlocks
                let project_root = {
                    let mut s = state.lock().unwrap();
                ...
                let runner = llm_agent::AgentRunner::new(state.clone());
...
fn draw_tui(f: &mut Frame<'_>, area: Rect, state: &AppState) {
    ...
    if state.input_mode {
        ...
        // Position actual terminal cursor at end of input text
        let cursor_x = bottom_chunks[1].x + 3 + state.cursor_pos as u16;
        let cursor_y = bottom_chunks[1].y + 1;
        #[allow(deprecated)]
                ...
                Event::Mouse(mouse) => {
                    ...
                    match mouse.kind {
                        MouseEventKind::ScrollUp => {
                            if state.input_mode && !state.input_buf.is_empty() {
