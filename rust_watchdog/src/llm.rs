...
pub async fn call_qwen(prompt: &str) -> Result<Option<String>> {
    ...
    let resp = client
        ...
        .context("iflow request failed")?;

    if !resp.status().is_success() {
        return Ok(None);
    }
    let v: serde_json::Value = resp.json().await.map_err(anyhow::Error::new).context("iflow parse json")?;
    // try common shapes
    if let Some(text) = v
